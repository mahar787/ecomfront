...
const page = () => {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    await postReq(`${process.env.NEXT_PUBLIC_DOMAIN_URL}/api/authUser`, data)
      .then((data) => {
        if (data.statusCode == 200) {
          setApiResponse(data.response.data);
          localStorage.setItem("zxcvbnm", data.response.token);
          ...
          <div className="w-full sm:w-3/5">
            <div className="p-8">
              ...
              <p className="mt-2 mb-5 text-base leading-tight ">
                Login to get access to unlimited products
              </p>
              <form className="mt-8" onSubmit={handleSubmit(onSubmit)}>
                <div className="relative mt-2 w-full">
                  ...
                  <label
                    htmlFor="username"
                    className="absolute top-2 left-1 z-10 origin-[0] -translate-y-4 scale-75 transform cursor-text select-none  px-2 text-sm  duration-300 peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:scale-100 peer-focus:top-2 peer-focus:-translate-y-4 peer-focus:scale-75 peer-focus:px-2 peer-focus:text-blue-300"
                  >
                    {" "}
                    Enter Your username{" "}
                  </label>
                ...
                <input
                  className="mt-4 w-full cursor-pointer rounded-lg bg-blue-300 pt-3 pb-3  shadow-lg hover:bg-blue-400"
                  type="submit"
